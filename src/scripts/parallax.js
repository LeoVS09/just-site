...
export default function dynamicParallax (parallax = PARALLAX, content = CONTENT, ratio = RATIO) {
  ...
  /* background fix */
  if (!tag('html')[0].matches('.touch')) { clas(parallax).forEach(el => (el.style.backgroundAttachment = 'fixed')) }
  ...
  const fullscreenFix = () =>
    ...
    clas(content).forEach(el =>
      (el.clientHeight > document.body.offsetHeight) &&
        (el.closest('.' + parallax).className += ' overflow')
    )
  ...
  function backgroundResize (ratio) {
    ...
    clas(parallax).forEach(function (path) {
      ...
      let remainingH = 0
      if (!tag('html')[0].matches('.touch')) { remainingH = windowH - contH }
